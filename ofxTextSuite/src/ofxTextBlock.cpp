    ...
    if (words.size() > 0) {
        ...
        for(int l=0;l < lines.size(); l++)
        {
            for(int w=0;w < lines[l].wordsID.size(); w++)
            {
                ...
                drawX = x + currX;
                drawY = y + (defaultFont.getLineHeight() * (l + 1 + deltaY));
...
////                    words.erase(words.begin()+ currentWordID);
//                    words[currentWordID].rawWord.pop_back();
//                    currX = 0;
//                 //   defaultFont.drawString("SALTO", drawX, drawY);
////                    drawY += 50;
////                    drawX = 0;
//                    deltaY += 1;
//                }else
                if (ofIsStringInString(words[currentWordID].rawWord, "*")){
                    words[currentWordID].rawWord.pop_back();
                    altFont.drawString(words[currentWordID].rawWord.c_str(), drawX, drawY);
                    currX += words[currentWordID].width * 0.95;
                }else{
